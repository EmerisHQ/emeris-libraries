<template>
  <div>
    <p v-for="item in pending" :key="item.id" v-on:click="() =>{ respond(item.id) }">{{ item }}</p>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const respond = (id) => {
      browser.runtime.sendMessage({ type: 'fromPopup', data: { action: 'setResponse', data: pending.value.find(item => item.id==id) } });
